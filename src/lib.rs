...
/// The reponse returned by a state handler function.
...
/// Trait that must be implemented for the event that will be handled by
/// the state machine.
...
/// Trait to be implemented on your struct.
pub trait Stateful: Sized {
    ...
    fn init(&mut self) {
        ...
        let mut entry_temp = *self.state_mut();
